...
// Connect provides a mock function with given fields: username, password, twoFactorSecret
func (_m *GamecoordinatorUseCase) Connect(username string, password string, twoFactorSecret string) {
	_m.Called(username, password, twoFactorSecret)
}
...
func (_m *GamecoordinatorUseCase) RequestMatchDetails(_a0 *share_code.ShareCodeData) chan *shared.MatchDetails {
	ret := _m.Called(_a0)

	var r0 chan *shared.MatchDetails
	if rf, ok := ret.Get(0).(func(*share_code.ShareCodeData) chan *shared.MatchDetails); ok {
		r0 = rf(_a0)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(chan *shared.MatchDetails)
		}
	}
