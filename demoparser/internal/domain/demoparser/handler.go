...
func (s *ParserService) handleMatchStart(e events.MatchStart) {
...
func (s *ParserService) handleGamePhaseChanged(e events.GamePhaseChanged) {
...
func (s *ParserService) handleRoundStart(e events.RoundStart) {
	if s.RoundOngoing {
		return
	}
...
func (s *ParserService) handleMVP(e events.RoundMVPAnnouncement) {
...
func (s *ParserService) handleRoundEnd(e events.RoundEnd) {
	if !s.RoundOngoing {
		return
	}
...
func (s *ParserService) handleKill(e events.Kill) {
...
func (s *ParserService) handlePlayerHurt(e events.PlayerHurt) {
	if s.parser.GameState().IsWarmupPeriod() || s.CurrentRound == 0 {
		return
	}
...
func (s *ParserService) handleRankUpdate(e events.RankUpdate) {
	player, err := s.getPlayer(e.Player)
	if err != nil {
		log.Error(err)
	}
...
func (s *ParserService) handleParserWarn(e events.ParserWarn) {
