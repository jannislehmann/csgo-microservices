...
func (suite *MainTestSuite) TestUpdateUserApiCredentials() {
	suite.userHandlerMock.On("UpdateUserApiCredentials", mock.Anything, mock.Anything).Return(&pb.StatusResponse{}, nil)
	...
	defer conn.Close()
	client := pb.NewUserServiceClient(conn)
	resp, err := client.UpdateUserApiCredentials(ctx, &pb.UpdateUserApiCredentialsRequest{})
	if err != nil {
		suite.T().Fatalf("GetUser failed: %v", err)
	}
	log.Printf("Response: %+v", resp)
